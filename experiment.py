...
def controller(data_in):
    ...
    with tf.variable_scope((PREFIX_CONTROLLER + EXTENSION_NUMBER(2)), reuse=CONTROLLER_INITIALIZED) as scope:
        ...
        linear_w_content = initialize_weights_cpu(
            ...
            [HIDDEN_SIZE, 1, MEMORY_LOCATIONS])
        ...
        linear_w_query = initialize_weights_cpu(
            (scope.name + EXTENSION_QUERY), 
            [HIDDEN_SIZE, MEMORY_DEPTH, MEMORY_LOCATIONS])
        linear_b_query = initialize_biases_cpu(
            (scope.name + EXTENSION_QUERY), 
            [MEMORY_LOCATIONS, MEMORY_DEPTH, 1])
        query = tf.add(tf.tensordot(tf.transpose(linear_w_query), activation, 1), linear_b_query)
    ...
    return location, content, query, output


def update_memory(key_batch, value_batch, query_batch):
    ...
    with tf.variable_scope((PREFIX_MEMORY + EXTENSION_NUMBER(1)), reuse=MEMORY_INITIALIZED) as scope:
        ...
        memory = memory + key_batch * tf.tile(value_batch, [1, MEMORY_DEPTH, 1])
        output = tf.reduce_sum(memory * query_batch, axis=1)
...
def train_model(num_epochs=1):
    ...
    with tf.Graph().as_default():
        ...
        for i in range(SEQUENCE_LENGTH):
            ...
            location, content, query, prediction = controller(
            ...
            feedback = update_memory(location, content, query)
